...
namespace Tetris
{
	public class TetrisField
	{
		...
		{
			TilesWidth=width; //ширина поля
			TilesHeight=height; //высота поля
			Tiles=new TileType[height, width];
			
			BorderColor=Color.FromArgb(192, 192, 192); //цвета границы тетриса
			BackColor=Color.FromArgb(240, 240, 240); //цвета поля тетриса
...
// --------------  МОДИФИКАЦИИ ЯЧЕЕК
		...
		protected bool MoveRight(int row, int col)
		{
			if(Tiles[row, col]!=TileType.Empty)
			{
				...
				return Tiles[row, col+1]==TileType.Empty; //Может двигаться вправо
			}
			return false;
		}
		/// <summary>
		...
		protected bool MoveLeft(int row, int col)
		{
			if(Tiles[row, col]!=TileType.Empty)
			{
				...
				return Tiles[row, col-1]==TileType.Empty; //Может продолжать дивгаться влево
			}
			return false;
		}
		/// <summary>
